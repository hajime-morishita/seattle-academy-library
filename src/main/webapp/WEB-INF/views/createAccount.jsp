            ...
            <div class="authorization">
                <div class="authorization_form">
                    <form method="post" action="createAccount">
                        ...
                        <label class="label">メールアドレス</label> <input type="text" class="input" id="email" name="email" autocomplete="off" required>
                        <c:if test="${!empty emailError}">
                            <div class="error">${emailError}</div>
                        </c:if>
                        <label class="label">パスワード</label> <input type="password" class="input" id="password" name="password" required> <label class="label">パスワード（確認用）</label> <input type="password" class="input" id="passwordForCheck" name="passwordForCheck" required>
                        <c:if test="${!empty passwordError}">
                            <div class="error">${passwordError}</div>
                        </c:if>
                        <c:if test="${!empty checkPasswordError}">
                            <div class="error">${checkPasswordError}</div>
                        </c:if>
                        <input type="submit" class="button primary" value="作成する">
                         <c:if test="${!empty mailPWCheck}">
                            <div class="error">${mailPWCheck}</div>
                        </c:if>
                    </form>
                </div>
